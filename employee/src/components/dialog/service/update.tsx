...
interface IUpdateServiceDialogProps {
    service: IService;
}

export default function UpdateServiceDialog({
    service,
}: IUpdateServiceDialogProps) {
    const navigate = useNavigate();
    const [open, SetOpen] = useState<boolean>(false);

    const handleConfirm = () => {
        navigate(`${ROUTER_PATH.SERVICE}/${service.id}`);
    };
    ...
    return (
        <Dialog open={open} onOpenChange={SetOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Chuyển tới trang cập nhật dịch vụ</DialogTitle>
                    <DialogDescription>

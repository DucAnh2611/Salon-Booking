...
import { IApiOrderListSort } from "@/interface/api/order.interface";
import SelectSort from "./select/select-sort";
interface ISheetSortOrderProductProps {
    sort: IApiOrderListSort;
    onConfirm: (sort: IApiOrderListSort) => void;
}

export default function SheetSortOrderProduct({
    sort,
    onConfirm,
}: ISheetSortOrderProductProps) {
    const [newSort, setNewSort] = useState<IApiOrderListSort>(sort);
    const [open, SetOpen] = useState<boolean>(false);

    const handleSelectSort =
        (field: keyof IApiOrderListSort) => (value: ESortBy | null) => {
            const newO = { ...newSort };
            if (!value) {
                delete newO[field];
            } else {

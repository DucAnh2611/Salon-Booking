...
import { IServiceMedia } from "@/interface/service.interface";
...
interface IServiceDetailMediaProps {
    medias: IServiceMedia[];
}

export default function ServiceDetailMedia({
    medias,
}: IServiceDetailMediaProps) {
    ...
    return (
        <div className="w-full h-fit grid grid-cols-5 gap-2">
            <div className="flex flex-col gap-2 col-span-1 overflow-auto h-auto">
                {medias.map((media, i) => (
                    <div
                        key={
                            media.media
